...
const newScheduleArray:timeSchedule[]=[];
  ...
  for (let k=0;k<result[j][1].length;k++){
    ...
    newScheduleArray.push(hour,minutes,event)
  }
newIcalCalenderArray.push(date,newScheduleArray)
}
...
const makeIcaleArray=(Date:number,Hour:number,Minutes:number,Event:string):OwnCalendar[]=>{
  const newIcalArray:OwnCalendar[]=[];
  newIcalArray.push({Date:Date,Hour:Hour,Minutes:Minutes,Event:Event});
  return newIcalArray;
}



const makeScheduleArray=(Hour:number,Minutes:number,Event:string):timeSchedule[]=>{
  const newScheduleArray:timeSchedule[]=[];
  newScheduleArray.push({Hour:Hour,Minutes:Minutes,Event:Event});
